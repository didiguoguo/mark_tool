export default function Editor(canvas, img) {
...
Editor.prototype.init = function (canvas, tools, img) {
    this.canvas = canvas
    this.img = img
...
//鼠标离开区域事件
Editor.prototype.onMouseOut = function (e) {
...
//切换操作模式
Editor.prototype.toggleActionMode = function (mode) {
    this.paintType = mode
...
//过滤未完成多边形
Editor.prototype.filterUncompletedPolygon = function () {
    this.shapes = this.shapes.filter(item => item.paintType !== 3 || (item.paintType === 3 && item.isCompleted))
}


// 绘制多边形时  检查当前线段是否与已有线段交叉
// Editor.prototype.toggleActionMode = function (points, point) {

// }

// 动态设置鼠标样式
Editor.prototype.checkCurrentPoint = function (points, cPoint) {
...
// 检查当前点是否在形状内
Editor.prototype.checkInShape = function (shape, point) {
    let distance
...
// 检查是否在图形边缘
Editor.prototype.checkInShapeBorder = function (shape, point) {
    let x1 = shape.x
    let x2 = point.x
    let y1 = shape.y
    let y2 = point.y
...
//切换颜色
Editor.prototype.onColorChange = function (v) {
    this.isPainting = false
    this.filterUncompletedPolygon()
...
//缩放
Editor.prototype.zoom = function (tag) {
...
//绘制圆形
Editor.prototype.drawCircle = function (circle) {
    let {
        x,
        y,
        r,
...
//绘制线段
Editor.prototype.drawLine = function (sPoint, ePoint, color, selected) {
    sPoint = {
        ...sPoint,
...
//绘制多边形
Editor.prototype.drawBrokenLine = function (polygon) {
    let {
        points,
        color,
        isCompleted,
...
//绘制当前图形
Editor.prototype.drawOwnImage = function () {
...
//绘制所有图形
Editor.prototype.drawShapes = function () {
